...
def getSingleComm(G, partition,_uid):
    numcomm = 0
    for e in partition:
        if partition[e] > numcomm:
            numcomm = partition[e]
    groups = [[] for i in range(numcomm+1)]
    for e in partition:
        groups[partition[e]].append(e)
    SGraph = None
    index = -1
    min_ne = 1000000
    ...
    return SGraph, index, numcomm


def get_clusters(G,_uid=None):
    ...
    Notes
    -----
    Uses Louvain algorithm version 2
    ...
    group, index, numcomm = getSingleComm(G,partition,_uid)
    H = G.subgraph(group)
    # get 2nd partition
    partition2 = best_partition(H)
    # merget 2 partition
    for e in partition2:
        if partition2[e] != 0:
            partition[e] = numcomm + partition2[e]
    # return result
    return partition
